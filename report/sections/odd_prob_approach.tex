...
Soient alors les ensembles $R'_{k} \subset \{2\mathbb{N}+1 \cup \{2\}\}$ tels que $\forall~i \in \{2\mathbb{N}+1 \cup \{2\}\}$, le probabilité que $i$ soit dans l'ensemble $R'_{k}$ soit:
...
Le fait que $P(i \in R'_{k}) = 1$ pour les entiers impairs inférieurs à 9 découle du au fait que $2/\log(n) > 1$ pour ces entiers.
...
Pour tout $n \in 2\mathbb{N}+1, n\geq 5$, les fonctions $\sigma_{R'_k}$ sont donc des variables aléatoires. Pour simplifier l'écriture dans les sommations ci-dessous, nous posons, pour tout $n \in 2\mathbb{N} + 1, m:= \lfloor 2/n \rfloor$, de sorte que $2m-1$ soit bien le plus grand entier impair inférieur à $n$.
Notons $\sigma_{R'}$ la fonction mesurant l'espérance des fonctions $\sigma_{R'_k}$ en fonction de $n$.
\[
\sigma_{R'}(n)
...
\end{aligned} \]
La première inégalité résulte de l'inégalité triangulaire et de $Li(9) > 4$. La seconde inégalité vient du fait que $1/\log(x)$ est positive et décroissante sur l'intervalle $[5,\infty[$, 
